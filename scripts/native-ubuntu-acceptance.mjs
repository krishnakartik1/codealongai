...
try {
  ...
  const packagedManifest = async (name) => { let directory = dirname(packagedRequire.resolve(name)); while (true) { try { const manifest = JSON.parse(await readFile(join(directory, 'package.json'), 'utf8')); if (manifest.name === name) return manifest; } catch {} const parent = dirname(directory); if (parent === directory) throw new Error('packaged dependency missing'); directory = parent; } };
