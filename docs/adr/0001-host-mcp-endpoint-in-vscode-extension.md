...
The extension switch starts and stops the endpoint with the extension lifecycle. The endpoint remains discoverable while enabled when no walkthrough session exists and reports that no walkthrough is active. The hackathon assumes one VS Code window; multi-window endpoint coordination is deferred.
