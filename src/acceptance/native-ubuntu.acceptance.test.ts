...
async function settled(read: () => readonly ProducerObservation[], label: string): Promise<readonly ProducerObservation[]> { const completed = await eventually(() => { const events = read(); return events.some((event) => event.kind === 'terminal-done') && events.some((event) => event.kind === 'receipt-matched') && events.some((event) => event.kind === 'session-deleted') ? events : undefined; }, `${label} did not settle with a terminal receipt and session cleanup.`); assert.equal(completed.some((event) => event.kind === 'forbidden'), false, `${label} used a forbidden capability.`); assert.equal(completed.find((event) => event.kind === 'call')?.name, 'codealongai_get_walkthrough_request', `${label} must read request authority first.`); return completed; }
...
suite('Native Ubuntu production acceptance', () => {
  test('proves a model-backed Ask and graph-producing Reply without changing the workspace', async function (this: Mocha.Context) {
    ...
    const askEvents = await settled(() => api.producerObservations.slice(askStart), 'Ask');
    ...
    const replyEvents = await settled(() => api.producerObservations.slice(replyStart), 'Reply');
