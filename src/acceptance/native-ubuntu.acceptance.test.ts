...
import { localSandboxRuntimeDirectoryCount, validTurnCallSequence } from './native-ubuntu-preflight';
...
suite('Native Ubuntu production acceptance', () => {
  test('proves a model-backed Ask and graph-producing Reply without changing the workspace', async function (this: Mocha.Context) {
    ...
    const store = process.env.CODEALONGAI_TRUEFORGE_DATA_PATH; assert.ok(store, 'CODEALONGAI_TRUEFORGE_DATA_PATH is required.'); const localSandboxBefore = await localSandboxRuntimeDirectoryCount(store);
    ...
    assert.equal(await localSandboxRuntimeDirectoryCount(store), localSandboxBefore, 'Daytona turns must not create local Sandbox Runtime directories.');
