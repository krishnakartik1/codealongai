...
export class InteractionController {
  ...
  public advance(): InteractionState { if (this.current.follow === 'awaiting-consent') return this.current; const event = this.replay.advance(); if (!event) return this.current; const aiAttention = { name: 'CodeAlongAI' as const, target: event.target }; if (event.kind === 'propose') { this.current = { ...this.current, aiAttention, proposalCaptureTarget: event.target }; return this.current; } if (event.kind === 'explain') { const explanation = { message: event.message, target: event.target }; if (event.target.document !== this.current.humanSelection?.document) { this.pendingExplanation = explanation; this.current = { ...this.current, aiAttention, follow: 'awaiting-consent', followTarget: event.target }; return this.current; } this.current = { ...this.current, aiAttention, explanations: [...this.current.explanations, explanation] }; return this.current; } this.current = { ...this.current, aiAttention }; return this.current; }
