...
export class WalkthroughAuthority {
  ...
  /** A question transition remains private until the producer receipt is
   * accepted.  The committed session is therefore never used as scratch
   * space by an unacknowledged producer turn. */
  private tentativeQuestion: { readonly commit: QuestionCommit; readonly outcome: QuestionOutcome; readonly base: WalkthroughSession; readonly candidate: WalkthroughSession; readonly receipt: QuestionReceipt } | undefined;
  ...
  /** Replacement has the same receipt boundary as start: its candidate never
   * becomes the active walkthrough until the producer observes its receipt. */
  private tentativeReplacement: { readonly origin: OriginDescriptor; readonly base: WalkthroughSession; readonly candidate: WalkthroughSession; readonly receipt: SessionReceipt } | undefined;
  ...
  public discardReplacement(requestId?: string): void { if (this.replacementRequest?.status === 'pending' && (requestId === undefined || this.replacementRequest.id === requestId)) { this.replacementRequest.status = 'cancelled'; if (this.tentativeReplacement?.receipt.requestId === this.replacementRequest.id) this.tentativeReplacement = undefined; } }
  public replace(requestId: string, expectedSessionId: string, expectedRevision: number, origin: OriginDescriptor): SessionReceipt {
    const request = this.replacementRequests.get(requestId); const session = this.session; const accepted = this.sessionReceipts.get(requestId); const tentative = this.tentativeReplacement;
    if (accepted && (!session || session.id === accepted.sessionId)) return { ...accepted };
    if (tentative?.receipt.requestId === requestId) { if (JSON.stringify(tentative.origin) !== JSON.stringify(origin) || tentative.base.id !== expectedSessionId || tentative.base.revision !== expectedRevision) throw new Error('walkthrough replacement does not match its tentative receipt'); return { ...tentative.receipt }; }
    if (!request || request.status !== 'pending' || !session || request.expectedSessionId !== expectedSessionId || request.expectedRevision !== expectedRevision || session.id !== expectedSessionId || session.revision !== expectedRevision) throw new Error('walkthrough replacement is unavailable or stale');
    if (!sameAnchor(request.origin, origin)) throw new Error('origin does not match the authorized replacement request');
    const candidate = this.newSession(origin); const receipt: SessionReceipt = { schemaVersion: 1, status: 'committed', requestId, sessionId: candidate.id, revision: candidate.revision, attentionStopId: candidate.attentionStopId };
    this.tentativeReplacement = { origin: { ...origin, range: copyRange(origin.range) }, base: copySession(session), candidate, receipt };
    return { ...receipt };
  }
  public acknowledgeReplacementReceipt(receipt: SessionReceipt): boolean { const request = this.replacementRequest; const tentative = this.tentativeReplacement; const session = this.session; if (!request || request.status !== 'pending' || !tentative || !session || JSON.stringify(tentative.receipt) !== JSON.stringify(receipt) || session.id !== tentative.base.id || session.revision !== tentative.base.revision || session.attentionStopId !== tentative.base.attentionStopId) return false; request.status = 'consumed'; this.session = tentative.candidate; this.sessionReceipts.set(request.id, tentative.receipt); this.tentativeReplacement = undefined; return true; }
  public rollbackTentativeReplacement(receipt?: SessionReceipt): boolean { const tentative = this.tentativeReplacement; if (!tentative || (receipt && JSON.stringify(tentative.receipt) !== JSON.stringify(receipt))) return false; this.tentativeReplacement = undefined; return true; }
  ...
  public discardQuestion(requestId?: string): void { if (this.questionRequest?.status === 'pending' && (requestId === undefined || this.questionRequest.id === requestId)) { this.questionRequest.status = 'cancelled'; if (this.tentativeQuestion?.commit.requestId === this.questionRequest.id) this.tentativeQuestion = undefined; } }
  /** Returns an exact terminal or staged receipt without re-running any
   * producer-supplied validation.  Changed retries remain conflicts. */
  public cachedQuestionReceipt(commit: QuestionCommit, outcome: QuestionOutcome): QuestionReceipt | undefined {
    const terminal = this.questionReceipts.get(commit.requestId);
    if (terminal && JSON.stringify(terminal.commit) === JSON.stringify(commit) && JSON.stringify(terminal.outcome) === JSON.stringify(outcome)) return copyReceipt(terminal.receipt);
    const tentative = this.tentativeQuestion;
    return tentative && JSON.stringify(tentative.commit) === JSON.stringify(commit) && JSON.stringify(tentative.outcome) === JSON.stringify(outcome) ? copyReceipt(tentative.receipt) : undefined;
  }
  public commitQuestionOutcome(commit: QuestionCommit, outcome: QuestionOutcome): QuestionReceipt { const terminal = this.questionReceipts.get(commit.requestId); if (terminal) { if (JSON.stringify(terminal.commit) !== JSON.stringify(commit) || JSON.stringify(terminal.outcome) !== JSON.stringify(outcome)) throw new Error('question request does not match its terminal receipt'); return copyReceipt(terminal.receipt); } const tentative = this.tentativeQuestion; if (tentative) { if (JSON.stringify(tentative.commit) !== JSON.stringify(commit) || JSON.stringify(tentative.outcome) !== JSON.stringify(outcome)) throw new Error('question request does not match its tentative receipt'); return copyReceipt(tentative.receipt); } const request = this.questionRequests.get(commit.requestId); const session = this.session; if (!request || request.status !== 'pending' || !session || request.sessionId !== commit.sessionId || session.id !== commit.sessionId || session.revision !== commit.revision) throw new Error('question request is unavailable or stale'); const stops = session.stops.map(copyStop); if (outcome.kind === 'generated-walkthrough') applyPatch(stops, outcome.patch); if (outcome.kind === 'destination-offer') validateOffer(stops, request.sourceStopId, outcome.destinationIds); const source = stops.find((stop) => stop.id === request.sourceStopId); if (!source) throw new Error('question source is unavailable'); source.conversation.push({ author: 'You', bodyMarkdown: request.text }, { author: 'CodeAlongAI', bodyMarkdown: outcome.answerMarkdown }); validateGraph(stops, session.origin.stopId); const candidate = { ...session, revision: session.revision + 1, stops }; const receipt: QuestionReceipt = { schemaVersion: 1, status: 'committed', requestId: commit.requestId, sessionId: commit.sessionId, revision: candidate.revision, attentionStopId: candidate.attentionStopId }; this.tentativeQuestion = { commit: { ...commit }, outcome: copyOutcome(outcome), base: copySession(session), candidate: copySession(candidate), receipt }; return copyReceipt(receipt); }
  public acknowledgeQuestionReceipt(receipt: QuestionReceipt): boolean { const tentative = this.tentativeQuestion; const request = this.questionRequest; const session = this.session; if (!tentative || !request || request.status !== 'pending' || JSON.stringify(tentative.receipt) !== JSON.stringify(receipt) || !session || session.id !== tentative.base.id || session.revision !== tentative.base.revision || session.attentionStopId !== tentative.base.attentionStopId) return false; request.status = 'consumed'; this.session = copySession(tentative.candidate); this.questionReceipts.set(request.id, { commit: tentative.commit, outcome: tentative.outcome, receipt: tentative.receipt }); this.tentativeQuestion = undefined; return true; }
  public rollbackTentativeQuestion(receipt?: QuestionReceipt): boolean { const tentative = this.tentativeQuestion; if (!tentative || (receipt && JSON.stringify(tentative.receipt) !== JSON.stringify(receipt))) return false; this.tentativeQuestion = undefined; return true; }
