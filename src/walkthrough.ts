...
export class WalkthroughAuthority {
  ...
  public start(requestId: string, origin: OriginDescriptor): WalkthroughSession { const request = this.startRequest?.id === requestId ? this.startRequest : undefined; if (!request || request.status !== 'pending') throw new Error('start request is unavailable'); if (!sameAnchor(request.origin, origin)) throw new Error('origin does not match the authorized request'); this.session = this.newSession(origin); request.status = 'consumed'; return this.getSession()!; }
