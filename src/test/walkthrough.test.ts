...
suite('receipt-backed start producer turn', () => {
  ...
  test('accepts only a correlated start receipt after matching request authority', () => {
    const reducer = new ProducerTurnReducer('request-1');
  ...
  test('uses the smallest exclusive origin line interval for multi-line authority ranges', () => {
    const accept = (endCharacter: number, readEndLine: number): ProducerTurnReducer => {
      const reducer = new ProducerTurnReducer('request-1');
  ...
  test('fails an out-of-order tool call and refuses sandbox command events', () => {
    ...
    const command = new ProducerTurnReducer('request-1');
  ...
  test('requires a successful authority result before bounded origin context', () => {
    ...
    const malformed = new ProducerTurnReducer('request-1');
  ...
  test('applies one absolute deadline to stalled create, turn, and stream operations', async () => {
    ...
    assert.deepEqual(await new ReceiptBackedProducerCoordinator(base({ runTurn: async () => never, cancelTurn: async () => { cleanup.push('cancel'); }, deleteSession: async () => { cleanup.push('delete'); } }), 5).start(input), { status: 'failed', diagnostic: 'deadline_exceeded' });
    assert.deepEqual(cleanup, ['cancel', 'delete']);
    cleanup.length = 0;
    assert.deepEqual(await new ReceiptBackedProducerCoordinator(base({ events: async function* (_s, _t, _after, options) { await Promise.race([never, waitForAbort(options?.abortSignal)]); }, cancelTurn: async () => { cleanup.push('cancel'); }, deleteSession: async () => { cleanup.push('delete'); } }), 5).start(input), { status: 'failed', diagnostic: 'deadline_exceeded' });
    assert.deepEqual(cleanup, ['cancel', 'delete']);
  });
  ...
  test('keeps a live receipt authoritative when persisted reconciliation replays its stable tool-call event', async () => {
    ...
    const runtime: TrueForgeProducerRuntime = { ...emptyTrueForgeProducer, createSession: async () => ({ id: 'session' }), runTurn: async () => ({ id: 'turn' }), events: async function* (_sessionId, _turnId, after) { if (after === undefined) { yield call('authority', 'codealongai_get_walkthrough_request', { requestId: 'request-1' }, 1); yield response('authority', { input: { origin: { path: 'checkout.ts', range: { start: { line: 2, character: 0 }, end: { line: 2, character: 0 } } } } }, 2); yield call('origin', 'codealongai_read_workspace_file', { path: 'checkout.ts', startLine: 2, endLine: 2 }, 3); yield response('origin', { structuredContent: { schemaVersion: 1, path: 'checkout.ts', startLine: 2, endLine: 2, text: 'x' } }, 4); yield start; yield response('stable-start-call', receipt, 6); return; } assert.equal(after, 7); yield { sequenceNumber: 8, event: { type: 'turn.done', id: 'terminal-done', state: { status: 'done' } } }; }, listTurnEvents: async () => { reconciliations += 1; return [{ ...start, sequenceNumber: 7 }]; } };
    const coordinator = new ReceiptBackedProducerCoordinator(runtime, 1_000, async () => new Promise<void>(() => undefined));
    assert.deepEqual(await coordinator.start({ requestId: 'request-1', configuration: { model: 'openai/gpt', reasoningEffort: 'medium', mcpUrl: 'unused' }, observe: (event) => observations.push(event) }), { status: 'committed', receipt });
    assert.deepEqual(await coordinator.settled, { status: 'committed', receipt });
