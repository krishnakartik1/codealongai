...
suite('receipt-backed start producer turn', () => {
  ...
  test('keeps a live receipt authoritative when persisted reconciliation replays its stable tool-call event', async () => {
    ...
    let observedLiveStart!: () => void;
    const liveStartObserved = new Promise<void>((resolve) => { observedLiveStart = resolve; });
    const runtime: TrueForgeProducerRuntime = { ...emptyTrueForgeProducer, createSession: async () => ({ id: 'session' }), runTurn: async () => ({ id: 'turn' }), events: async function* () { yield call('authority', 'codealongai_get_walkthrough_request', { requestId: 'request-1' }, 1); yield response('authority', { input: { origin: { path: 'checkout.ts', range: { start: { line: 2, character: 0 }, end: { line: 2, character: 0 } } } } }, 2); yield call('origin', 'codealongai_read_workspace_file', { path: 'checkout.ts', startLine: 2, endLine: 2 }, 3); yield response('origin', { structuredContent: { schemaVersion: 1, path: 'checkout.ts', startLine: 2, endLine: 2, text: 'x' } }, 4); yield start; await liveStartObserved; yield response('stable-start-call', receipt, 6); }, listTurnEvents: async () => [{ ...start, sequenceNumber: 7 }, { sequenceNumber: 8, event: { type: 'turn.done', id: 'terminal-done', state: { status: 'done' } } }] };
    assert.deepEqual(await new ReceiptBackedProducerCoordinator(runtime, 1_000, async () => new Promise<void>(() => undefined)).start({ requestId: 'request-1', configuration: { model: 'openai/gpt', reasoningEffort: 'medium', mcpUrl: 'unused' }, observe: (event) => { observations.push(event); if (event.kind === 'call' && event.name === 'codealongai_start_walkthrough') observedLiveStart(); } }), { status: 'committed', receipt });
